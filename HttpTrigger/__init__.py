# HttpTrigger/__init__.py (Refactorizado - Final Corregido v2)
...
import os
import io
import inspect # Para revisar signatura de funciones
...
try:
    ...
    logger.critical(f"Error crítico: No se pudo importar desde 'shared.constants'. Verifica estructura. {e}")
...
try:
    ...
    # SharePoint
    from actions.sharepoint import (crear_lista as sp_crear_lista, listar_listas as sp_listar_listas, agregar_elemento as sp_agregar_elemento_lista, listar_elementos as sp_listar_elementos_lista, actualizar_elemento as sp_actualizar_elemento_lista, eliminar_elemento as sp_eliminar_elemento_lista, listar_documentos_biblioteca as sp_listar_documentos_biblioteca, subir_documento as sp_subir_documento, eliminar_archivo as sp_eliminar_archivo_biblioteca, crear_carpeta_biblioteca as sp_crear_carpeta_biblioteca, mover_archivo as sp_mover_archivo, copiar_archivo as sp_copiar_archivo, obtener_metadatos_archivo as sp_obtener_metadatos_archivo, actualizar_metadatos_archivo as sp_actualizar_metadatos_archivo, obtener_contenido_archivo as sp_obtener_contenido_archivo, actualizar_contenido_archivo as sp_actualizar_contenido_archivo, crear_enlace_compartido_archivo as sp_crear_enlace_compartido_archivo)
    # Teams
    from actions.teams import (listar_chats as team_listar_chats, obtener_chat as team_obtener_chat, crear_chat as team_crear_chat, enviar_mensaje_chat as team_enviar_mensaje_chat, obtener_mensajes_chat as team_obtener_mensajes_chat, actualizar_mensaje_chat as team_actualizar_mensaje_chat, eliminar_mensaje_chat as team_eliminar_mensaje_chat, listar_equipos as team_listar_equipos, obtener_equipo as team_obtener_equipo, crear_equipo as team_crear_equipo, archivar_equipo as team_archivar_equipo, unarchivar_equipo as team_unarchivar_equipo, eliminar_equipo as team_eliminar_equipo, listar_canales as team_listar_canales, obtener_canal as team_obtener_canal, crear_canal as team_crear_canal, actualizar_canal as team_actualizar_canal, eliminar_canal as team_eliminar_canal, enviar_mensaje_canal as team_enviar_mensaje_canal)
...
except ImportError as e: logger.error(f"Error al importar módulos de acciones: {e}. Verifica estructura.", exc_info=True); ALL_ACTIONS_LOADED = False
except Exception as e: logger.error(f"Error inesperado durante importación: {e}", exc_info=True); ALL_ACTIONS_LOADED = False
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    funcion_a_ejecutar: Optional[Callable[..., Any]] = None
    resultado: Any = None
    ...
    try:
        ...
        request_headers = {'Authorization': auth_header, 'Content-Type': 'application/json'}
        ...
        if accion in acciones_disponibles:
            ...
            try:
                ...
                for param_name, param_type in type_hints.items():
                     ...
                     if param_name in params_procesados and params_procesados[param_name] is not None:
                         ...
                         try:
                             ...
                             elif target_type is list and isinstance(original_value, str): params_procesados[param_name] = json.loads(original_value)
                             elif target_type is dict and isinstance(original_value, str): params_procesados[param_name] = json.loads(original_value)
                ...
                if accion in upload_actions:
                    ...
                    # Renombrar/Asegurar params necesarios para funciones
                    if accion == "sp_actualizar_contenido_archivo":
                        params_procesados['nuevo_contenido'] = params_procesados.pop('contenido_bytes')
                    # Asegurar nombre de archivo
                    if 'nombre_archivo' not in params_procesados:
                         original_name = params_procesados.get('nombre_archivo_original')
                         if original_name: params_procesados['nombre_archivo'] = original_name
                         # Considerar error si no hay nombre original ni nombre_archivo
                         elif 'nombre_archivo_destino' in params_procesados: # Usado en algunas firmas originales
                              params_procesados['nombre_archivo'] = params_procesados.pop('nombre_archivo_destino')
                         else:
                              raise ValueError(f"Acción '{accion}' requiere 'nombre_archivo' o 'nombre_archivo_destino'.")
            ...
            try:
                ...
                logger.info(f"Invocation {invocation_id}: Ejecución de '{accion}' completada.")
                 ...
                 if 'headers' not in sig.parameters: logger.error(f"Error: Falta 'headers' en {funcion_a_ejecutar.__name__}!", exc_info=True); return func.HttpResponse(f"Error interno.", status_code=500)
                 else: logger.error(f"Error argumento {funcion_a_ejecutar.__name__}: {type_err}. Params: {params_procesados}", exc_info=True); return func.HttpResponse(f"Error argumentos '{accion}': {type_err}", status_code=400)
            except Exception as exec_err:
                logger.exception(f"Invocation {invocation_id}: Error ejecución '{accion}': {exec_err}")
                ...
                else: error_msg = f"Error interno: {exec_err}"[:500]
            ...
            if isinstance(resultado, bytes):
                 ...
                 mimetype = "application/octet-stream"; logger.info(f"Devolviendo {len(resultado)} bytes. Filename: {filename}"); return func.HttpResponse(resultado, mimetype=mimetype, headers={'Content-Disposition': f'attachment; filename="{filename}"'}, status_code=200)
            elif isinstance(resultado, (dict, list)):
                 logger.info(f"Devolviendo resultado JSON.");
                 ...
                 logger.info(f"Devolviendo resultado como texto plano."); return func.HttpResponse(str(resultado), mimetype="text/plain", status_code=200)
