...
# Importar constantes globales desde shared
try:
    ...
    logger.warning("No se pudo importar BASE_URL/GRAPH_API_TIMEOUT desde shared (Calendario), usando defaults.")
...
def crear_evento(
    ...
    if not isinstance(inicio, datetime) or not isinstance(fin, datetime): raise ValueError("'inicio' y 'fin' deben ser objetos datetime.")
    inicio = _ensure_timezone(inicio); fin = _ensure_timezone(fin)
    body: Dict[str, Any] = {"subject": titulo, "start": {"dateTime": inicio.isoformat(), "timeZone": "UTC"}, "end": {"dateTime": fin.isoformat(), "timeZone": "UTC"},}
...
def actualizar_evento(headers: Dict[str, str], evento_id: str, nuevos_valores: Dict[str, Any], mailbox: str = 'me') -> Dict[str, Any]:
    ...
    if 'start' in payload and isinstance(payload.get('start'), datetime): start_dt = _ensure_timezone(payload['start']); payload['start'] = {"dateTime": start_dt.isoformat(), "timeZone": "UTC"}
    if 'end' in payload and isinstance(payload.get('end'), datetime): end_dt = _ensure_timezone(payload['end']); payload['end'] = {"dateTime": end_dt.isoformat(), "timeZone": "UTC"}
    response: Optional[requests.Response] = None
    try:
