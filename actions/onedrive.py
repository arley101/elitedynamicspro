...
try:
    ...
    logger.error("Error importando helpers en OneDrive.")
    def hacer_llamada_api(*args, **kwargs): raise NotImplementedError("Helper no importado")

# ---- Helpers Locales ----
def _get_od_me_drive_endpoint() -> str: return f"{BASE_URL}/me/drive"
def _get_od_me_item_path_endpoint(ruta_relativa: str) -> str:
    drive_endpoint = _get_od_me_drive_endpoint(); safe_path = ruta_relativa.strip()
...
def listar_archivos(headers: Dict[str, str], ruta: str = "/", top: int = 100) -> dict:
    item_endpoint = _get_od_me_item_path_endpoint(ruta); url_base = f"{item_endpoint}/children"; params: Dict[str, Any] = {'$top': min(int(top), 999)}; all_items: List[Dict[str, Any]] = []; current_url: Optional[str] = url_base; current_headers = headers.copy(); response: Optional[requests.Response] = None
    ...
    except requests.exceptions.RequestException as e: logger.error(f"Error Request en od_listar_archivos: {e}", exc_info=True); raise Exception(f"Error API listando archivos OD: {e}")
...
def subir_archivo(headers: Dict[str, str], nombre_archivo: str, contenido_bytes: bytes, ruta: str = "/", conflict_behavior: str = "rename") -> dict:
    target_folder_path = ruta.strip('/'); target_file_path = f"/{nombre_archivo}" if not target_folder_path else f"/{target_folder_path}/{nombre_archivo}"; item_endpoint = _get_od_me_item_path_endpoint(target_file_path); url = f"{item_endpoint}:/content"; params = {"@microsoft.graph.conflictBehavior": conflict_behavior}; upload_headers = headers.copy(); upload_headers['Content-Type'] = 'application/octet-stream';
    logger.info(f"Subiendo OD /me '{nombre_archivo}' a ruta '{ruta}'")
    if len(contenido_bytes) > 4*1024*1024: logger.warning(f"Archivo OD '{nombre_archivo}' > 4MB.")
    # Usar helper para subida con 'data'
    return hacer_llamada_api("PUT", url, upload_headers, params=params, data=contenido_bytes, timeout=GRAPH_API_TIMEOUT * 3)

def descargar_archivo(headers: Dict[str, str], nombre_archivo: str, ruta: str = "/") -> bytes:
    target_folder_path = ruta.strip('/'); target_file_path = f"/{nombre_archivo}" if not target_folder_path else f"/{target_folder_path}/{nombre_archivo}"; item_endpoint = _get_od_me_item_path_endpoint(target_file_path); url = f"{item_endpoint}/content";
    logger.info(f"Descargando OD /me '{nombre_archivo}' de ruta '{ruta}'")
    # Usar requests directo para obtener bytes
    try:
    ...
    except requests.exceptions.RequestException as e: logger.error(f"Error Request en od_descargar_archivo: {e}", exc_info=True); raise Exception(f"Error API descargando archivo OD: {e}")
    except Exception as e: logger.error(f"Error inesperado en od_descargar_archivo (/me): {e}", exc_info=True); raise

def eliminar_archivo(headers: Dict[str, str], nombre_archivo: str, ruta: str = "/") -> Optional[Dict[str, Any]]:
    target_folder_path = ruta.strip('/'); target_file_path = f"/{nombre_archivo}" if not target_folder_path else f"/{target_folder_path}/{nombre_archivo}"; item_endpoint = _get_od_me_item_path_endpoint(target_file_path); url = item_endpoint;
    logger.info(f"Eliminando OD /me '{nombre_archivo}' de ruta '{ruta}'")
...
def crear_carpeta(headers: Dict[str, str], nombre_carpeta: str, ruta: str = "/", conflict_behavior: str = "rename") -> dict:
    parent_folder_endpoint = _get_od_me_item_path_endpoint(ruta); url = f"{parent_folder_endpoint}/children"; body: Dict[str, Any] = {"name": nombre_carpeta, "folder": {}, "@microsoft.graph.conflictBehavior": conflict_behavior};
    logger.info(f"Creando OD /me carpeta '{nombre_carpeta}' en ruta '{ruta}'")
...
def mover_archivo(headers: Dict[str, str], nombre_archivo: str, ruta_origen: str = "/", ruta_destino: str = "/NuevaCarpeta", nuevo_nombre: Optional[str] = None) -> dict:
    ...
    parent_reference_path = "/drive/root" if parent_dest_path == '/' else f"/drive/root:{parent_dest_path}"; body: Dict[str, Any] = { "parentReference": { "path": parent_reference_path } }; body["name"] = nuevo_nombre if nuevo_nombre is not None else nombre_archivo;
    logger.info(f"Moviendo OD /me '{nombre_archivo}' de '{ruta_origen}' a '{ruta_destino}'")
...
def copiar_archivo(headers: Dict[str, str], nombre_archivo: str, ruta_origen: str = "/", ruta_destino: str = "/Copias", nuevo_nombre_copia: Optional[str] = None) -> dict:
    ...
    try: drive_data = hacer_llamada_api("GET", f"{drive_endpoint}?$select=id", headers); actual_drive_id = drive_data.get('id'); assert actual_drive_id is not None
    ...
    if not parent_dest_path.startswith('/'): parent_dest_path = '/' + parent_dest_path
    parent_reference_path = "/drive/root" if parent_dest_path == '/' else f"/drive/root:{parent_dest_path}"; body: Dict[str, Any] = {"parentReference": { "driveId": actual_drive_id, "path": parent_reference_path }}; body["name"] = nuevo_nombre_copia if nuevo_nombre_copia is not None else f"Copia de {nombre_archivo}";
    logger.info(f"Iniciando copia OD /me '{nombre_archivo}' de '{ruta_origen}' a '{ruta_destino}'")
    # La copia devuelve 202 con Location header, el helper no lo maneja. Llamada directa:
    try:
    ...
    except requests.exceptions.RequestException as e: logger.error(f"Error Request en od_copiar_archivo: {e}", exc_info=True); raise Exception(f"Error API iniciando copia OD: {e}")
...
def obtener_metadatos_archivo(headers: Dict[str, str], nombre_archivo: str, ruta: str = "/") -> dict:
    target_folder_path = ruta.strip('/'); item_path = f"/{nombre_archivo}" if not target_folder_path else f"/{target_folder_path}/{nombre_archivo}"; item_endpoint = _get_od_me_item_path_endpoint(item_path); url = item_endpoint;
    logger.info(f"Obteniendo metadatos OD /me '{nombre_archivo}' ruta '{ruta}'")
...
def actualizar_metadatos_archivo(headers: Dict[str, str], nombre_archivo: str, nuevos_valores: dict, ruta:str = "/") -> dict:
    target_folder_path = ruta.strip('/'); item_path = f"/{nombre_archivo}" if not target_folder_path else f"/{target_folder_path}/{nombre_archivo}"; item_endpoint = _get_od_me_item_path_endpoint(item_path); url = item_endpoint;
    logger.info(f"Actualizando metadatos OD /me '{nombre_archivo}' ruta '{ruta}'")
    current_headers = headers.copy(); etag = nuevos_valores.pop('@odata.etag', None)
