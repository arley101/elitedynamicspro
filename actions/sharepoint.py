...
def listar_elementos_lista(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    try:
        while current_url and page_count < max_pages:
            page_count += 1; logger.info(f"Listando elementos SP lista '{lista_id_o_nombre}', P{page_count}")
            current_params = params_query if page_count == 1 else None; assert current_url is not None
            data = hacer_llamada_api("GET", current_url, headers, params=current_params)
            if data: page_items = data.get('value', []); all_items.extend(page_items); current_url = data.get('@odata.nextLink');
    ...
    except requests.exceptions.RequestException as e: logger.error(f"Error Request listar_elementos_lista p{page_count}: {e}", exc_info=True); raise Exception(f"Error API listando elementos SP: {e}") from e
...
def eliminar_elemento_lista(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    lista_id_o_nombre: Optional[str] = parametros.get("lista_id_o_nombre"); item_id: Optional[str] = parametros.get("item_id"); etag: Optional[str] = parametros.get("etag")
    if not lista_id_o_nombre: raise ValueError("'lista_id_o_nombre' requerido."); if not item_id: raise ValueError("'item_id' requerido.")
    target_site_id = _obtener_site_id_sp(parametros, headers); url = f"{BASE_URL}/sites/{target_site_id}/lists/{lista_id_o_nombre}/items/{item_id}"
...
def listar_documentos_biblioteca(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    url_base = f"{item_endpoint}/children"; params_query = {'$top': min(top, 999)}; all_files: List[Dict[str, Any]] = []; current_url: Optional[str] = url_base; page_count = 0; max_pages = 100
    try:
        while current_url and page_count < max_pages:
            page_count += 1; target_drive_name = biblioteca or SHAREPOINT_DEFAULT_DRIVE_ID or 'Documents'; logger.info(f"Listando docs SP biblio '{target_drive_name}', Ruta: '{ruta_carpeta}', P{page_count}")
            current_params = params_query if page_count == 1 else None; assert current_url is not None
            data = hacer_llamada_api("GET", current_url, headers, params=current_params)
            if data: page_items = data.get('value', []); all_files.extend(page_items); current_url = data.get('@odata.nextLink');
            else: logger.warning(f"Llamada a {current_url} devolvió None/vacío."); break
            if not current_url: break
        if page_count >= max_pages: logger.warning(f"Límite de {max_pages} páginas alcanzado.")
        logger.info(f"Total docs/carpetas SP encontrados: {len(all_files)}"); return {'value': all_files}
    except requests.exceptions.RequestException as e: logger.error(f"Error Request listar_documentos_biblioteca p{page_count}: {e}", exc_info=True); raise Exception(f"Error API listando docs SP: {e}") from e
    except Exception as e: logger.error(f"Error inesperado listar_documentos_biblioteca p{page_count}: {e}", exc_info=True); raise
...
def subir_documento(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    biblioteca: Optional[str] = parametros.get("biblioteca"); ruta_carpeta_destino: str = parametros.get("ruta_carpeta_destino", '/'); conflict_behavior: str = parametros.get("conflict_behavior", "rename")
    if not nombre_archivo: raise ValueError("'nombre_archivo' requerido."); if contenido_bytes is None or not isinstance(contenido_bytes, bytes): raise ValueError("'contenido_bytes' (bytes) requerido.")
    ...
    item_endpoint = _get_sp_item_path_endpoint(target_site_id, target_file_path, target_drive); file_size_mb = len(contenido_bytes) / (1024 * 1024)
    ...
    if file_size_mb > 4.0:
        ...
        try:
            logger.info(f"Archivo > 4MB. Creando sesión carga..."); session_info = hacer_llamada_api("POST", create_session_url, headers, json_data=session_body); upload_url = session_info.get("uploadUrl");
            if not upload_url: raise ValueError("No se pudo obtener 'uploadUrl'."); logger.info(f"Sesión creada. URL: {upload_url[:50]}...")
            chunk_size = 5 * 1024 * 1024; start_byte = 0; total_bytes = len(contenido_bytes); last_response_json = {}
            while start_byte < total_bytes:
                end_byte = min(start_byte + chunk_size - 1, total_bytes - 1); chunk_data = contenido_bytes[start_byte : end_byte + 1]; content_range = f"bytes {start_byte}-{end_byte}/{total_bytes}"; chunk_headers = {'Content-Length': str(len(chunk_data)), 'Content-Range': content_range}
                logger.debug(f"Subiendo chunk SP: {content_range}"); chunk_timeout = max(GRAPH_API_TIMEOUT, int(file_size_mb * 5)); chunk_response = requests.put(upload_url, headers=chunk_headers, data=chunk_data, timeout=chunk_timeout)
                chunk_response.raise_for_status(); start_byte = end_byte + 1
                if chunk_response.content: try: last_response_json = chunk_response.json(); except json.JSONDecodeError: pass
        ...
        url_put_simple = f"{item_endpoint}:/content"; params_query = {"@microsoft.graph.conflictBehavior": conflict_behavior}; upload_headers = headers.copy(); upload_headers['Content-Type'] = 'application/octet-stream';
        try: simple_upload_timeout = max(GRAPH_API_TIMEOUT, int(file_size_mb * 10)); resultado = hacer_llamada_api("PUT", url_put_simple, upload_headers, params=params_query, data=contenido_bytes, timeout=simple_upload_timeout, expect_json=True)
...
def eliminar_archivo_biblioteca(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    logger.info(f"Eliminando archivo/carpeta SP '{item_path}' en biblio '{target_drive}'"); hacer_llamada_api("DELETE", url, headers); return {"status": "Eliminado", "path": item_path}

def crear_carpeta_biblioteca(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    nombre_carpeta: Optional[str] = parametros.get("nombre_carpeta"); biblioteca: Optional[str] = parametros.get("biblioteca"); ruta_carpeta_padre: str = parametros.get("ruta_carpeta_padre", '/'); conflict_behavior: str = parametros.get("conflict_behavior", "rename")
    if not nombre_carpeta: raise ValueError("'nombre_carpeta' requerido."); target_site_id = _obtener_site_id_sp(parametros, headers); target_drive = biblioteca or SHAREPOINT_DEFAULT_DRIVE_ID or 'Documents'
    parent_folder_endpoint = _get_sp_item_path_endpoint(target_site_id, ruta_carpeta_padre, target_drive); url = f"{parent_folder_endpoint}/children"
...
def mover_archivo_biblioteca(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    if not nombre_archivo_o_carpeta: raise ValueError("'nombre_archivo_o_carpeta' requerido."); if nueva_ruta_carpeta_padre is None: raise ValueError("'nueva_ruta_carpeta_padre' requerido.")
...
def copiar_archivo_biblioteca(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    if not nombre_archivo: raise ValueError("'nombre_archivo' requerido."); if nueva_ruta_carpeta_padre is None: raise ValueError("'nueva_ruta_carpeta_padre' requerido.")
...
def actualizar_contenido_archivo_biblioteca(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    target_folder_path = ruta_carpeta.strip('/'); item_path = f"/{nombre_archivo}" if not target_folder_path else f"/{target_folder_path}/{nombre_archivo}"
    item_endpoint = _get_sp_item_path_endpoint(target_site_id, item_path, target_drive); url = f"{item_endpoint}/content";
    upload_headers = headers.copy(); upload_headers['Content-Type'] = 'application/octet-stream'; file_size_mb = len(nuevo_contenido_bytes) / (1024 * 1024)
    logger.info(f"Actualizando contenido SP '{item_path}' ({file_size_mb:.2f} MB)")
    if file_size_mb > 4.0: logger.warning(f"Actualizando archivo SP > 4MB con PUT simple."); # Podría fallar
    try:
        update_timeout = max(GRAPH_API_TIMEOUT, int(file_size_mb * 10)); resultado = hacer_llamada_api("PUT", url, upload_headers, data=nuevo_contenido_bytes, timeout=update_timeout, expect_json=True)
...
def crear_enlace_compartido_archivo_biblioteca(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    logger.info(f"Creando enlace SP (tipo: {tipo_enlace}, alcance: {alcance}) para '{item_path}'"); return hacer_llamada_api("POST", url, headers, json_data=body)

# ======================================================
# ==== FUNCIONES DE MEMORIA PERSISTENTE (LISTA SP) ====
# ======================================================
def _ensure_memory_list_exists(headers: Dict[str, str], site_id: str) -> bool:
    try: list_url = f"{BASE_URL}/sites/{site_id}/lists/{MEMORIA_LIST_NAME}?$select=id"; hacer_llamada_api("GET", list_url, headers); logger.debug(f"Lista memoria '{MEMORIA_LIST_NAME}' existe."); return True
    except requests.exceptions.RequestException as e:
        if e.response is not None and e.response.status_code == 404:
            logger.info(f"Lista memoria '{MEMORIA_LIST_NAME}' no encontrada. Creando..."); columnas = [{"name": "SessionID", "text": {}, "indexed": True}, {"name": "Clave", "text": {}, "indexed": True}, {"name": "Valor", "text": {"allowMultipleLines": True, "textType": "plain"}}, {"name": "Timestamp", "dateTime": {}, "indexed": True}]
...
def guardar_dato_memoria(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    if not session_id: raise ValueError("'session_id' requerido."); if not clave: raise ValueError("'clave' requerido."); if valor is None: raise ValueError("'valor' requerido (no None).")
    target_site_id = _obtener_site_id_sp(parametros, headers);
    ...
    filter_query = f"fields/SessionID eq '{session_id}' and fields/Clave eq '{clave}'"; params_listar = {"lista_id_o_nombre": MEMORIA_LIST_NAME, "site_id": target_site_id, "filter_query": filter_query, "top": 1, "select": "id,@odata.etag"}
    ...
    if item_id:
        ...
        logger.info(f"Guardando nuevo dato memoria: Session={session_id}, Clave={clave}"); params_agregar = {"lista_id_o_nombre": MEMORIA_LIST_NAME, "datos_campos": datos_campos, "site_id": target_site_id}
...
def recuperar_datos_sesion(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    if not session_id: raise ValueError("'session_id' requerido."); target_site_id = _obtener_site_id_sp(parametros, headers)
    ...
    for item in items_data.get("value", []):
        ...
        elif clave_any: logger.warning(f"Clave inválida encontrada en memoria: {clave_any}")
...
def eliminar_dato_memoria(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    if not session_id: raise ValueError("'session_id' requerido."); if not clave: raise ValueError("'clave' requerido.")
    target_site_id = _obtener_site_id_sp(parametros, headers);
    if not _ensure_memory_list_exists(headers, target_site_id): return {"status": "Lista no encontrada"}
    filter_query = f"fields/SessionID eq '{session_id}' and fields/Clave eq '{clave}'"; params_listar = {"lista_id_o_nombre": MEMORIA_LIST_NAME, "site_id": target_site_id, "filter_query": filter_query, "top": 1, "select": "id"}
    ...
    if item_id:
        logger.info(f"Eliminando memoria: Session={session_id}, Clave={clave}"); params_eliminar = {"lista_id_o_nombre": MEMORIA_LIST_NAME, "item_id": item_id, "site_id": target_site_id}
...
def exportar_datos_lista(parametros: Dict[str, Any], headers: Dict[str, str]) -> Union[Dict[str, Any], str]:
    ...
    if not lista_id_o_nombre: raise ValueError("'lista_id_o_nombre' requerido."); if formato not in ["json", "csv"]: raise ValueError("Formato no soportado. Use 'json' o 'csv'.")
