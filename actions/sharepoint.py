"""
actions/sharepoint.py (Refactorizado y Corregido)
...
Corrección:
- Añadido 'from datetime import datetime, timezone' para resolver F821.
...
# CORRECCIÓN: Importar datetime y timezone
    ...
    try:
        while current_url and page_count < max_pages:
            ...
            if data:
                ...
                if not current_url: logger.debug("No hay '@odata.nextLink', se termina paginación."); break
            else: logger.warning(f"La llamada a {current_url} devolvió None o vacío. Terminando paginación."); break
        if page_count >= max_pages: logger.warning(f"Se alcanzó el límite de {max_pages} páginas al listar elementos de '{lista_id_o_nombre}'.")
        logger.info(f"Total elementos SP lista '{lista_id_o_nombre}': {len(all_items)}")
        return {'value': all_items}
    except requests.exceptions.RequestException as e: logger.error(f"Error Request en listar_elementos_lista (SP) página {page_count}: {e}", exc_info=True); raise Exception(f"Error API listando elementos SP: {e}") from e
    except Exception as e: logger.error(f"Error inesperado en listar_elementos_lista (SP) página {page_count}: {e}", exc_info=True); raise
...
def actualizar_elemento_lista(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    if etag: current_headers['If-Match'] = etag; logger.debug(f"Usando ETag '{etag}' para actualización concurrente.")
...
def eliminar_elemento_lista(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    if etag: current_headers['If-Match'] = etag; logger.debug(f"Usando ETag '{etag}' para eliminación concurrente.")
...
def listar_documentos_biblioteca(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    try:
        while current_url and page_count < max_pages:
            page_count += 1; target_drive_name = biblioteca or SHAREPOINT_DEFAULT_DRIVE_ID or 'Documents'; logger.info(f"Listando docs SP biblioteca '{target_drive_name}', Ruta: '{ruta_carpeta}', Página: {page_count}")
            current_params = params_query if page_count == 1 else None
            data = hacer_llamada_api("GET", current_url, headers, params=current_params)
            if data:
                page_items = data.get('value', [])
            ...
            else: logger.warning(f"Llamada a {current_url} para listar docs devolvió None/vacío."); break
        if page_count >= max_pages: logger.warning(f"Se alcanzó límite de {max_pages} páginas listando docs en '{ruta_carpeta}'.")
        logger.info(f"Total docs/carpetas SP encontrados: {len(all_files)}"); return {'value': all_files}
    except requests.exceptions.RequestException as e: logger.error(f"Error Request en listar_documentos_biblioteca (SP) página {page_count}: {e}", exc_info=True); raise Exception(f"Error API listando documentos SP: {e}") from e
    except Exception as e: logger.error(f"Error inesperado en listar_documentos_biblioteca (SP) página {page_count}: {e}", exc_info=True); raise
...
def subir_documento(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    if file_size_mb > 4.0:
        ...
        try:
            logger.info(f"Archivo > 4MB. Creando sesión de carga para '{nombre_archivo}'..."); session_info = hacer_llamada_api("POST", create_session_url, headers, json_data=session_body); upload_url = session_info.get("uploadUrl");
            if not upload_url: raise ValueError("No se pudo obtener 'uploadUrl' de la sesión de carga."); logger.info(f"Sesión de carga creada. URL: {upload_url[:50]}...");
            ...
            logger.info(f"Doc SP '{nombre_archivo}' subido exitosamente mediante sesión."); return last_response_json
        except requests.exceptions.RequestException as e: logger.error(f"Error Request durante sesión de carga SP para '{nombre_archivo}': {e}", exc_info=True); raise Exception(f"Error API durante sesión de carga SP: {e}") from e
        except Exception as e: logger.error(f"Error inesperado durante sesión de carga SP para '{nombre_archivo}': {e}", exc_info=True); raise
    else:
        try:
             simple_upload_timeout = max(GRAPH_API_TIMEOUT, int(file_size_mb * 10));
             resultado = hacer_llamada_api(metodo="PUT", url=url_put_simple, headers=upload_headers, params=params_query, data=contenido_bytes, timeout=simple_upload_timeout, expect_json=True)
             logger.info(f"Doc SP '{nombre_archivo}' subido (subida simple)."); return resultado
        except requests.exceptions.RequestException as e: logger.error(f"Error Request en subida simple SP de '{nombre_archivo}': {e}", exc_info=True); raise Exception(f"Error API subiendo documento SP (simple): {e}") from e
        except Exception as e: logger.error(f"Error inesperado en subida simple SP de '{nombre_archivo}': {e}", exc_info=True); raise
...
def eliminar_archivo(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    url = item_endpoint; logger.info(f"Eliminando archivo/carpeta SP '{item_path}' en biblioteca '{target_drive}'")
...
def crear_carpeta_biblioteca(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    url = f"{parent_folder_endpoint}/children"; body: Dict[str, Any] = {"name": nombre_carpeta, "folder": {}, "@microsoft.graph.conflictBehavior": conflict_behavior};
...
def mover_archivo(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    target_folder_path_origen = ruta_carpeta_origen.strip('/'); item_path_origen = f"/{nombre_archivo_o_carpeta}" if not target_folder_path_origen else f"/{target_folder_path_origen}/{nombre_archivo_o_carpeta}"
...
def copiar_archivo(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    target_folder_path_origen = ruta_carpeta_origen.strip('/'); item_path_origen = f"/{nombre_archivo}" if not target_folder_path_origen else f"/{target_folder_path_origen}/{nombre_archivo}"
    ...
    if not drive_id_destino:
        ...
        except Exception as drive_err: raise Exception(f"Error obteniendo Drive ID de origen para copiar: {drive_err}") from drive_err
    parent_dest_path = nueva_ruta_carpeta_padre.strip();
    if not parent_dest_path.startswith('/'): parent_dest_path = '/' + parent_dest_path
    ...
    if isinstance(response, requests.Response) and response.status_code == 202:
        ...
        return {"status": "Copia Iniciada", "status_code": response.status_code, "monitorUrl": monitor_url, "detail": "La copia se realiza en segundo plano. Usa la URL de monitorización."}
    elif isinstance(response, requests.Response): logger.error(f"Respuesta inesperada al iniciar copia SP: {response.status_code} {response.reason}."); raise Exception(f"Respuesta inesperada al iniciar copia SP: {response.status_code}")
    else: logger.error(f"Respuesta inesperada del helper al iniciar copia SP: {type(response)}"); raise Exception("Error interno al procesar la solicitud de copia.")
...
def obtener_metadatos_archivo(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    target_folder_path = ruta_carpeta.strip('/'); item_path = f"/{nombre_archivo_o_carpeta}" if not target_folder_path else f"/{target_folder_path}/{nombre_archivo_o_carpeta}"
    item_endpoint = _get_sp_item_path_endpoint(target_site_id, item_path, target_drive)
    url = item_endpoint; logger.info(f"Obteniendo metadatos SP '{item_path}'")
...
def actualizar_metadatos_archivo(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    target_folder_path = ruta_carpeta.strip('/'); item_path = f"/{nombre_archivo_o_carpeta}" if not target_folder_path else f"/{target_folder_path}/{nombre_archivo_o_carpeta}"
    item_endpoint = _get_sp_item_path_endpoint(target_site_id, item_path, target_drive)
    url = item_endpoint; current_headers = headers.copy(); body_data = nuevos_valores.copy(); etag = body_data.pop('@odata.etag', None)
    if etag: current_headers['If-Match'] = etag; logger.debug("Usando ETag para actualización de metadatos SP.")
...
def obtener_contenido_archivo(parametros: Dict[str, Any], headers: Dict[str, str]) -> bytes:
    ...
    item_endpoint = _get_sp_item_path_endpoint(target_site_id, item_path, target_drive)
    url = f"{item_endpoint}/content"; logger.info(f"Obteniendo contenido SP '{item_path}'")
    ...
    else: logger.error(f"Respuesta inesperada del helper al obtener contenido SP: {type(response)}"); raise Exception("Error interno al obtener contenido del archivo SP.")
...
def actualizar_contenido_archivo(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    if file_size_mb > 4.0: logger.warning(f"Actualizando archivo > 4MB ({nombre_archivo}) con PUT simple. Podría fallar.")
    try:
        update_timeout = max(GRAPH_API_TIMEOUT, int(file_size_mb * 10));
        resultado = hacer_llamada_api(metodo="PUT", url=url, headers=upload_headers, data=nuevo_contenido_bytes, timeout=update_timeout, expect_json=True)
        logger.info(f"Contenido SP '{item_path}' actualizado exitosamente."); return resultado
    except requests.exceptions.RequestException as e: logger.error(f"Error Request al actualizar contenido SP de '{item_path}': {e}", exc_info=True); raise Exception(f"Error API actualizando contenido SP: {e}") from e
    except Exception as e: logger.error(f"Error inesperado al actualizar contenido SP de '{item_path}': {e}", exc_info=True); raise
...
def _ensure_memory_list_exists(headers: Dict[str, str], site_id: str) -> bool:
    try:
        list_url = f"{BASE_URL}/sites/{site_id}/lists/{MEMORIA_LIST_NAME}?$select=id"; hacer_llamada_api("GET", list_url, headers); logger.debug(f"Lista de memoria '{MEMORIA_LIST_NAME}' ya existe."); return True
    except requests.exceptions.RequestException as e:
        if e.response is not None and e.response.status_code == 404:
            logger.info(f"Lista de memoria '{MEMORIA_LIST_NAME}' no encontrada. Intentando crearla..."); columnas = [{"name": "SessionID", "text": {}, "indexed": True}, {"name": "Clave", "text": {}, "indexed": True}, {"name": "Valor", "text": {"allowMultipleLines": True, "textType": "plain"}}, {"name": "Timestamp", "dateTime": {}, "indexed": True}]; params_crear = {"nombre_lista": MEMORIA_LIST_NAME, "columnas": columnas, "site_id": site_id};
            try: crear_lista(params_crear, headers); logger.info(f"Lista de memoria '{MEMORIA_LIST_NAME}' creada exitosamente."); return True
            except Exception as create_err: logger.critical(f"¡Fallo al crear lista de memoria '{MEMORIA_LIST_NAME}'!: {create_err}", exc_info=True); return False
        else: logger.error(f"Error verificando existencia de lista de memoria '{MEMORIA_LIST_NAME}': {e}", exc_info=True); return False
...
def guardar_dato_memoria(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    target_site_id = _obtener_site_id_sp(parametros, headers)
    if not _ensure_memory_list_exists(headers, target_site_id): raise Exception(f"No se pudo asegurar la existencia de la lista de memoria '{MEMORIA_LIST_NAME}'.")
    try:
        ...
        else: valor_str = str(valor); logger.warning(f"Guardando tipo no estándar '{type(valor)}' como string: {valor_str[:50]}...")
    except Exception as json_err: raise ValueError(f"Error al serializar el valor para la clave '{clave}': {json_err}") from json_err
    filter_query = f"fields/SessionID eq '{session_id}' and fields/Clave eq '{clave}'"; params_listar = {"lista_id_o_nombre": MEMORIA_LIST_NAME, "site_id": target_site_id, "filter_query": filter_query, "top": 1, "select": "id,@odata.etag"}; item_id: Optional[str] = None; item_etag: Optional[str] = None;
    try:
    ...
    except Exception as e: logger.warning(f"Error buscando item existente para memoria ({session_id}/{clave}): {e}. Se intentará crear.")
    datos_campos = {"SessionID": session_id, "Clave": clave, "Valor": valor_str, "Timestamp": datetime.utcnow().isoformat() + "Z"}; # CORREGIDO: Usar datetime importado
    if item_id:
        logger.info(f"Actualizando dato en memoria: Session={session_id}, Clave={clave}"); params_actualizar = {"lista_id_o_nombre": MEMORIA_LIST_NAME, "item_id": item_id, "nuevos_valores_campos": datos_campos, "site_id": target_site_id};
        ...
        logger.info(f"Guardando nuevo dato en memoria: Session={session_id}, Clave={clave}"); params_agregar = {"lista_id_o_nombre": MEMORIA_LIST_NAME, "datos_campos": datos_campos, "site_id": target_site_id};
...
def recuperar_datos_sesion(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    if not session_id: raise ValueError("Parámetro 'session_id' es requerido.")
    target_site_id = _obtener_site_id_sp(parametros, headers)
    if not _ensure_memory_list_exists(headers, target_site_id): logger.warning(f"Lista de memoria '{MEMORIA_LIST_NAME}' no encontrada al recuperar datos."); return {}
    ...
    logger.info(f"Recuperando datos de memoria para Session={session_id}"); params_listar = {"lista_id_o_nombre": MEMORIA_LIST_NAME, "site_id": target_site_id, "filter_query": filter_query, "expand_fields": True, "select": select_fields, "order_by": order_by, "top": 999};
    ...
    for item in items_data.get("value", []):
        ...
        if clave and valor_str:
            if clave not in memoria:
                ...
                except json.JSONDecodeError: memoria[clave] = valor_str
                except Exception as parse_err: logger.warning(f"Error parseando valor para clave '{clave}' (Session: {session_id}): {parse_err}. Guardando como string."); memoria[clave] = valor_str
...
def eliminar_dato_memoria(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    if not clave: raise ValueError("Parámetro 'clave' es requerido.")
    target_site_id = _obtener_site_id_sp(parametros, headers)
    if not _ensure_memory_list_exists(headers, target_site_id): logger.warning(f"Lista de memoria '{MEMORIA_LIST_NAME}' no encontrada al eliminar dato."); return {"status": "Lista no encontrada"}
    filter_query = f"fields/SessionID eq '{session_id}' and fields/Clave eq '{clave}'"; params_listar = {"lista_id_o_nombre": MEMORIA_LIST_NAME, "site_id": target_site_id, "filter_query": filter_query, "top": 1, "select": "id"}; item_id: Optional[str] = None;
    try:
        existing_items_data = listar_elementos_lista(params_listar, headers); existing_items = existing_items_data.get("value", []);
    ...
    if item_id:
        logger.info(f"Eliminando dato de memoria: Session={session_id}, Clave={clave}"); params_eliminar = {"lista_id_o_nombre": MEMORIA_LIST_NAME, "item_id": item_id, "site_id": target_site_id};
        return eliminar_elemento_lista(params_eliminar, headers)
    else: logger.warning(f"No se encontró dato para eliminar: Session={session_id}, Clave={clave}"); return {"status": "No encontrado", "session_id": session_id, "clave": clave}

def eliminar_memoria_sesion(parametros: Dict[str, Any], headers: Dict[str, str]) -> Dict[str, Any]:
    session_id: Optional[str] = parametros.get("session_id");
    if not session_id: raise ValueError("Parámetro 'session_id' es requerido.")
    target_site_id = _obtener_site_id_sp(parametros, headers)
    if not _ensure_memory_list_exists(headers, target_site_id): logger.warning(f"Lista de memoria '{MEMORIA_LIST_NAME}' no encontrada al eliminar sesión."); return {"status": "Lista no encontrada", "items_eliminados": 0}
    filter_query = f"fields/SessionID eq '{session_id}'"; logger.info(f"Listando TODOS los items de memoria para eliminar Session={session_id}"); params_listar = {"lista_id_o_nombre": MEMORIA_LIST_NAME, "site_id": target_site_id, "filter_query": filter_query, "select": "id", "top": 999};
    ...
    if not item_ids_to_delete: logger.info(f"No se encontraron datos para eliminar para Session={session_id}"); return {"status": "Sin datos", "items_eliminados": 0, "session_id": session_id}
    ...
    for item_id in item_ids_to_delete:
        ...
        except Exception as del_err: logger.error(f"Error eliminando item {item_id} de memoria para sesión {session_id}: {del_err}"); count_failed += 1
...
def exportar_datos_lista(parametros: Dict[str, Any], headers: Dict[str, str]) -> Union[Dict[str, Any], str]:
    ...
    target_site_id = _obtener_site_id_sp(parametros, headers); logger.info(f"Exportando datos de lista '{lista_id_o_nombre}' en formato {formato}");
    ...
    if not items: logger.warning(f"No se encontraron items para exportar en lista '{lista_id_o_nombre}'."); return {"value": []} if formato == "json" else ""
    field_data = [];
